...
sep = os.sep
# 全局统计结果
gather_file_path = f'message{sep}Tieba{sep}结果.txt'
gather_ads_file_path = f'message{sep}Tieba{sep}代售贴吧广告.txt'
special_file_path = f'message{sep}Tieba{sep}广告.txt'
special_tag = '*'

# 得到所有体型列表
with open(f'message{sep}Tieba{sep}input{sep}体型.txt', 'r', encoding='utf-8') as f:
...
def write_gather_time_ticks():
    ...
    with open(gather_file_path, 'a', encoding='utf-8') as f:
        f.write(ticks)
    with open(gather_ads_file_path, 'a', encoding='utf-8') as f:
        f.write(ticks)
            ...
            for bracket in key_bracket_list:
                ...
                if ((price == 0) or (price <= int(keywords[1]) and body == keywords[2])):
                    content_check_keywords(
                        source, content, keywords, note_dict, result_path)
                # elif price <= int(keywords[1]) and body == keywords[2]:
                #     content_check_keywords(source, content, keywords, note_dict)
...
def flicker(content, note_dict: dict, id_num: str, max_length=20):
    ...
    print(f'repeated content : {cut_str},id:{id_num}')
...
def print_info(source, content, keywords, result_path=gather_file_path, special_path=special_file_path, special_tag=special_tag):
    ''' 找到内容后输出并写入文件
    '''
    # write into the result
    id_num = keywords[0]
    if id_num[0] == special_tag:
        print('发现新广告')
    ...
    print(content)
    if content.find("【") < 0 or content.find("】") < 0:        # gather file
        print(content)
        num_id = filter(str.isdigit, id_num)
        tieba_info = models.tiebainfo(userid=''.join(list(num_id)),userbackup=id_num,webaddr=source,keywordlist=str(keywords),detail=content)
        tieba_info.save()
        with open(output_path, 'a', encoding='utf-8') as f:
            f.write(output_info)
    else:
        print(content)
        tieba_ad = models.tiebaads(webaddr=source,keywordlist=str(keywords),detail=content)
        tieba_ad.save()
        with open(gather_ads_file_path, 'a', encoding='utf-8') as f:
            f.write(output_info)
...
def get_time_str():
    ...
    return (now + diff).strftime('%Y-%m-%d %H:%M:%S')
...
def load_proxy_list(path=f'message{sep}Tieba{sep}proxy'):
    with open(path, 'r', encoding='utf-8') as f:
        text = f.read().split('\n')
    res = []
    for item in text:
...
def keywords_trans():
    with open(f'message{sep}Tieba{sep}input{sep}关键词.txt', 'r', encoding='utf-8') as f:
    ...
    with open(f'message{sep}Tieba{sep}input{sep}keywords.txt', 'w+', encoding='utf-8') as f:
        f.write('\n'.join(res))
